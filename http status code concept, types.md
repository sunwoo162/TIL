...
: 307 : Temporary Redirect (클라이언트가 요청한 리소스가 다른 URI에 있으며 이전 요청과 동일한 메소드를 사용해서 요청해야 할 때 서버가 클라이언트에 C청을 직접 보냄)
...
```
## 전체 코드
#### 1XX : Informational(정보제공)(클라이언트의 요청을 받았으며, 작업을 계속 진행하고 있다.)
```
정보전송 : 100 : Continue (클라이언트로 부터 일부 요청을 받았으면 나머지 정보를 계속 요청함),(계속)
임시응답 : 101 : Switching protocols(프로토콜 전환)
         : 102 : Processing (처리중)
```
#### 2XX : Success(성공)(클라이언트가 요청한 동작을 성공적으로 처리하였다)
```
...
```
     ...
     : 506 : Variant Also Negotiates (변형)
     : 507 : Insufficient Storage (용량 부족)
     : 508 : Loop Detected (루프 감지됨)
     : 511 : Nedtwork Authentication Required (네트워크 인증 필요)
   ```
